...
export function createAgentRouter(broadcastBrowserEvent: (sessionId: string, event: SessionEvent) => void) {
  ...
  async function requireRemoteBrowserAccess(req: any, res: any): Promise<{ userId: string } | null> {
    ...
    if (!allowed) {
      res.status(403).json({
        error: "Remote browser data access is disabled. Enable it in Privacy settings.",
        code: "REMOTE_BROWSER_DISABLED",
      });
      return null;
    }
