...
openAICodexOAuthRouter.get("/status", async (_req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: "No se pudo consultar el estado de ChatGPT OAuth" });
  }
});
...
openAICodexOAuthRouter.post("/start", async (req: Request, res: Response) => {
  try {
    ...
    const message = error instanceof Error ? error.message : "No se pudo iniciar ChatGPT OAuth";
...
openAICodexOAuthRouter.get("/flow/:flowId", (req: Request, res: Response) => {
  try {
    ...
    const message = error instanceof Error ? error.message : "No se pudo consultar el flujo OAuth";
...
openAICodexOAuthRouter.post("/complete", (req: Request, res: Response) => {
  try {
    ...
    const message = error instanceof Error ? error.message : "No se pudo completar ChatGPT OAuth";
    const statusCode = message.includes("no pertenece") ? 403 : 400;
    res.status(statusCode).json({ error: message });
  }
});
