...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      defaultSrc: ["'self'"],
      // In production, avoid 'unsafe-eval' (XSS hardening). Vite dev requires eval in some setups.
      scriptSrc: isProductionCsp
        ? ["'self'", "'unsafe-inline'", "https://cdn.jsdelivr.net", "https://accounts.google.com"]
        : ["'self'", "'unsafe-inline'", "'unsafe-eval'", "https://cdn.jsdelivr.net", "https://accounts.google.com"],
      ...
      connectSrc: isProductionCsp
        ? ["'self'", "https://api.x.ai", "https://generativelanguage.googleapis.com", "https://accounts.google.com", "wss:"]
        : ["'self'", "https://api.x.ai", "https://generativelanguage.googleapis.com", "https://accounts.google.com", "wss:", "ws:"],
...
(async () => {
  ...
  if (isProduction) {
    try {
      ...
      if (llmHealth.openai?.available) {
        log("✅ OpenAI LLM connected");
      }
      if (llmHealth.anthropic?.available) {
        log("✅ Anthropic (Claude) LLM connected");
      }
      if (llmHealth.deepseek?.available) {
        log("✅ DeepSeek LLM connected");
      }

      if (
        !llmHealth.xai?.available &&
        !llmHealth.gemini?.available &&
        !llmHealth.openai?.available &&
        !llmHealth.anthropic?.available &&
        !llmHealth.deepseek?.available
      ) {
