...
const DEFAULT_TOOL_POLICIES: Record<string, Partial<ToolPolicy>> = {
  ...
  // OpenClaw — Advanced Memory (hybrid vector+FTS search)
  openclaw_memory_search_advanced: {
    capabilities: ["reads_files"],
    allowedPlans: ["free", "pro", "admin"],
    requiresConfirmation: false,
    maxExecutionTimeMs: 30000,
  ...
  openclaw_memory_index_document: {
    capabilities: ["writes_files"],
    allowedPlans: ["free", "pro", "admin"],
    requiresConfirmation: false,
    maxExecutionTimeMs: 60000,
  ...
  // OpenClaw — Media Understanding (image/audio/video AI)
  openclaw_describe_image: {
  ...
  // OpenClaw — Auth Profiles (credential rotation management)
  openclaw_auth_profiles_list: {
    capabilities: [],
    allowedPlans: ["free", "pro", "admin"],
    requiresConfirmation: false,
    maxExecutionTimeMs: 10000,
  ...
  // OpenClaw — Additional core tools
  openclaw_exec: {
    capabilities: ["executes_code", "high_risk"],
    allowedPlans: ["free", "pro", "admin"],
    requiresConfirmation: false,
  ...
  openclaw_document_batch_ingest: {
    capabilities: ["writes_files", "long_running"],
    allowedPlans: ["free", "pro", "admin"],
    requiresConfirmation: false,
    maxExecutionTimeMs: 300000,
    maxRetries: 1,
