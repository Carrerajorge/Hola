...
async function executeWebTool(input: unknown, context: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    const request: RetrievalRequest = {
      ...
      includeScholar: validated.includeScholar ?? false,
      preferBrowser: privacy.remoteBrowserDataAccess ? (validated.preferBrowser ?? false) : false,
      allowBrowser: privacy.remoteBrowserDataAccess,
      extractReadable: validated.extractReadable ?? true,
      deduplicateByContent: validated.deduplicateByContent ?? true,
      minQualityScore: validated.minQualityScore ?? 0,
