...
const runner_1 = require("./steps/runner");
...
async function main() {
    const argv = (0, yargs_1.default)((0, helpers_1.hideBin)(process.argv))
        ...
        .help()
        .parseSync();
    const planPath = path_1.default.resolve(argv.plan);
    ...
    const workspace = await (0, policy_1.validateWorkspaceRoot)(selectedWorkspace);
    const logPath = argv.log
        ? path_1.default.resolve(String(argv.log))
        : path_1.default.join(workspace, ".iliagpt", "logs", `${runId}.jsonl`);
    const options = {
        planPath,
        workspace,
        ...
        logPath,
        allowlistPath: argv.allowlist ? path_1.default.resolve(String(argv.allowlist)) : undefined,
    ...
    const logger = (0, logger_1.createLogger)(logPath);
    const telemetry = (0, telemetry_1.createTelemetry)(options.otelEndpoint);
    ...
    const results = await (0, runner_1.runPlan)({
        plan: runPlanPayload,
        workspace,
        stream: options.stream,
        dryRun: options.dryRun,
