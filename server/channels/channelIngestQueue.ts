...
export async function submitChannelIngest(job: ChannelIngestJob): Promise<void> {
  const useQueue = shouldUseQueue();

  if (useQueue && channelIngestQueue) {
    await channelIngestQueue.add("ingest", job, {
  ...
  setImmediate(() => {
    processChannelIngestJob(job).catch((err) => {
