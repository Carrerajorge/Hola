...
import { telegramSendDocument, telegramSendMessage, telegramSendPhoto } from "./telegram/telegramApi";
import {
...
const TELEGRAM_LOCAL_CONTROL_ATTACHMENT_MAX_BYTES = 49 * 1024 * 1024;
const TELEGRAM_AUTO_LOCAL_CONTROL_ENABLED = (() => {
  const raw = String(process.env.ILIAGPT_TELEGRAM_AUTO_LOCAL_CONTROL ?? "true").trim().toLowerCase();
  return !["0", "false", "off", "no"].includes(raw);
})();
const TELEGRAM_LOCAL_CONTROL_IMAGE_EXTENSIONS = new Set([
  ".png",
  ".jpg",
  ".jpeg",
  ".webp",
...
function sanitizeOutboundRichText(value: string): string {
  return String(value ?? "")
    .normalize("NFC")
    .replace(OUTBOUND_UNSAFE_CONTROL_RE, "")
    .replace(OUTBOUND_BIDI_CONTROL_RE, "")
    .replace(/\r\n/g, "\n")
    .replace(/\r/g, "\n");
}

function sanitizeStreamChunk(value: string, limit = MAX_STREAM_CHUNK_LENGTH): string {
  return enforceSafeLimit(sanitizeOutboundRichText(value), limit);
}
...
async function runOutboundDecision(
  ...
  const safeAssistantContent = enforceSafeLimit(sanitizeOutboundRichText(String(assistantContent ?? "")).trim(), MAX_OUTBOUND_TEXT_LENGTH);
  ...
  if (context.jobChannel === "telegram") {
    try {
      await maybeSendAutoGeneratedWordDocument(context, safeAssistantContent, safeRunId);
    } catch (error) {
      Logger.warn("[Channels] failed to auto-generate Telegram DOCX", {
        runId: safeRunId,
        conversation: context.envelope.conversationKey,
        traceId: safeTraceId,
        reason: String((error as Error)?.message || error),
      });
    }
  ...
  if (options?.assistantMessageId) {
    ...
    if (options.assistantMetadata && typeof options.assistantMetadata === "object") {
      Object.assign(metadata, options.assistantMetadata);
    }
...
function compactDocumentWords(text: string): string[] {
  return String(text || "")
    .normalize("NFKC")
    .replace(/[\r\n]+/g, " ")
...
function sanitizeGeneratedFileName(name: string, fallback: string): string {
...
function buildAutoWordTitle(context: InboundProcessingContext): string {
  const conversationTitle = compactDocumentWords(
    String((context.conversation as unknown as { title?: string })?.title || ""),
  ).slice(0, 6).join(" ");
  if (conversationTitle) return conversationTitle;

  const inboundTitle = compactDocumentWords(context.envelope.text).slice(0, 8).join(" ");
  if (inboundTitle) return inboundTitle;

  return "Documento IliaGPT";
}

async function maybeSendAutoGeneratedWordDocument(
  context: InboundProcessingContext,
  assistantContent: string,
  runId: string,
): Promise<void> {
  if (context.jobChannel !== "telegram") return;
  if (!shouldAutoGenerateWordDocument(context.envelope.text)) return;

  const content = assistantContent.trim();
  if (!content) return;

  const title = buildAutoWordTitle(context);
  const safeTitle = sanitizeGeneratedFileName(title, "Documento_IliaGPT").replace(/\.[^.]+$/, "");
  const fileName = sanitizeGeneratedFileName(`${safeTitle}_${runId}.docx`, `documento_${Date.now()}.docx`);

  const buffer = await generateWordDocument(title, content);
  await telegramSendDocument(context.envelope.threadId, buffer, fileName, WORD_DOCX_MIME);
}

function isTelegramLocalControlCandidate(text: string): boolean {
  const trimmed = String(text || "").trim();
  if (!trimmed) return false;
  return (
    /^\/local\b/i.test(trimmed) ||
    /^local:/i.test(trimmed) ||
    /^(?:deteneroff|deteneron)\b/i.test(trimmed)
  );
}

type TelegramLocalControlResult = {
  handled: true;
  ok: boolean;
  statusCode: number;
  code: string;
...
async function tryExecuteTelegramLocalControl(
  text: string,
  context: { requestId: string; userId?: string | null; traceId: string },
): Promise<TelegramLocalControlResult | null> {
  const explicitCommand = isTelegramLocalControlCandidate(text);
  if (!explicitCommand && !TELEGRAM_AUTO_LOCAL_CONTROL_ENABLED) return null;

  try {
    const { executeLocalControlRequest } = await import("../routes/chatAiRouter");
    const result = await executeLocalControlRequest(text, {
      requestId: context.requestId,
      userId: context.userId,
    });
    if (!result.handled) return null;
    return result as TelegramLocalControlResult;
  } catch (error) {
    Logger.error("[Channels] telegram local-control execution failed", {
      requestId: context.requestId,
    ...
    return {
      ...
      message: "No pude ejecutar la acción local en este momento.",
    };
  }
}
...
async function sendTelegramAttachmentFromLocalControlPayload(
  recipientId: string,
  payload: Record<string, unknown> | undefined,
  traceId: string,
): Promise<void> {
  const localPathRaw = String(payload?.path || "").trim();
  if (!localPathRaw) return;

  const filePath = path.resolve(localPathRaw);
  let stat;
  try {
  ...
  if (stat.size <= 0 || stat.size > TELEGRAM_LOCAL_CONTROL_ATTACHMENT_MAX_BYTES) {
    Logger.warn("[Channels] telegram local-control attachment skipped by size", {
      traceId,
      filePath,
      bytes: stat.size,
    });
    if (stat.size > TELEGRAM_LOCAL_CONTROL_ATTACHMENT_MAX_BYTES) {
      await telegramSendMessage(
        recipientId,
        `⚠️ Generé el archivo, pero pesa ${formatAttachmentBytes(stat.size)} y supera el límite de envío (${formatAttachmentBytes(TELEGRAM_LOCAL_CONTROL_ATTACHMENT_MAX_BYTES)}).`,
      ).catch(() => undefined);
    }
    return;
  }

  const fileName = String(payload?.fileName || path.basename(filePath)).slice(0, 180) || path.basename(filePath);
  const mimeType = String(payload?.mimeType || inferMimeTypeFromPath(filePath)).trim() || "application/octet-stream";
  const buffer = await fs.readFile(filePath);
  const ext = path.extname(fileName).toLowerCase();
  const isImage = mimeType.startsWith("image/") || TELEGRAM_LOCAL_CONTROL_IMAGE_EXTENSIONS.has(ext);

  if (isImage) {
    try {
      await telegramSendPhoto(recipientId, buffer, fileName, mimeType);
      return;
    } catch (error) {
      Logger.warn("[Channels] telegram photo send failed, retrying as document", {
        traceId,
        fileName,
        mimeType,
        reason: String((error as Error)?.message || error),
  ...
  await telegramSendDocument(recipientId, buffer, fileName, mimeType);
}
...
function truncateLocalRecordText(value: unknown, maxLen = 220): string | undefined {
  if (typeof value !== "string" && typeof value !== "number" && typeof value !== "boolean") {
    return undefined;
  }
  const text = String(value).normalize("NFKC").replace(/[\x00-\x1f\x7f]/g, "").trim();
  if (!text) return undefined;
  return text.length > maxLen ? `${text.slice(0, maxLen)}…` : text;
}

function buildLocalControlRecordLine(result: { ok: boolean; code?: string; payload?: Record<string, unknown> }): string {
  const code = truncateLocalRecordText(result.code, 80) || "UNKNOWN";
  const path = truncateLocalRecordText(result.payload?.path, 140);
  const appName = truncateLocalRecordText(result.payload?.appName, 80);
  const base = `[registro] accion=${code} ok=${result.ok ? "true" : "false"} ts=${new Date().toISOString()}`;
  if (path) return `${base} path=${path}`;
  if (appName) return `${base} app=${appName}`;
  return base;
}

function buildLocalControlAssistantMetadata(
  result: { ok: boolean; code?: string; payload?: Record<string, unknown> },
  promptText: string,
): Record<string, unknown> {
  const payloadSource = result.payload && typeof result.payload === "object" ? result.payload : {};
  ...
  for (const key of keys) {
    const rawValue = payloadSource[key];
    if (typeof rawValue === "number" && Number.isFinite(rawValue)) {
      payload[key] = rawValue;
      continue;
    }
    const safeValue = truncateLocalRecordText(rawValue, key === "path" ? 280 : 180);
    if (safeValue) payload[key] = safeValue;
  }
  return {
    localControl: {
      ok: !!result.ok,
      code: truncateLocalRecordText(result.code, 80) || "UNKNOWN",
      prompt: truncateLocalRecordText(promptText, 280) || "",
      payload,
    },
  };
}
...
async function processAllowedMessage(context: InboundProcessingContext): Promise<void> {
  ...
  if (pairingCode) {
    ...
    if (consumed?.userId) {
      ...
      // Enable mirror auto-reply immediately after successful pairing so
      // Telegram can drive the same ILIA chat from mobile without extra setup.
      await patchConversationMetadata(conversation.id, {
        runtime: {
          responder_enabled: true,
          owner_only: true,
          owner_external_ids: [safeEnvelope.senderId],
        },
        policy: {
  ...
  try {
    ...
    const localControlResult =
      jobChannel === "telegram"
        ? await tryExecuteTelegramLocalControl(userPromptText, {
            requestId: safeScopedRequestId,
    ...
    if (localControlResult?.handled) {
      const baseLocalReplyText = localControlResult.ok
        ? localControlResult.message
        : `No se pudo ejecutar la acción local: ${localControlResult.message}`;
      const localRecordLine = buildLocalControlRecordLine(localControlResult);
      const localReplyText = `${baseLocalReplyText}\n${localRecordLine}`.trim();
      const localAssistantMetadata = buildLocalControlAssistantMetadata(localControlResult, userPromptText);

      const assistantPlaceholder = await withTimeoutGuard(
        "channel_create_assistant_placeholder_local_control",
        ENFORCED_STORAGE_OPERATION_TIMEOUT_MS,
      ...
      await withTimeoutGuard(
        "channel_update_run_assistant_message_local_control",
        ENFORCED_STORAGE_OPERATION_TIMEOUT_MS,
        async () => await storage.updateChatRunAssistantMessage(safeRunId, assistantMessageId!),
      ).catch((error) => {
        Logger.warn("[Channels] failed to attach local-control assistant placeholder to run", {
          runId: safeRunId,
          conversation: safeEnvelope.conversationKey,
          traceId: eventTraceId,
          reason: String((error as Error)?.message || error),
      ...
      await runOutboundDecision(
        ...
        {
          assistantMessageId,
          assistantMetadata: localAssistantMetadata,
          traceId: eventTraceId,
          abortSignal: runAbort.signal,
        },
      );

      if (jobChannel === "telegram" && localControlResult.ok) {
        await sendTelegramAttachmentFromLocalControlPayload(
          safeEnvelope.threadId,
          localControlResult.payload,
          eventTraceId,
        ).catch((error) => {
          Logger.warn("[Channels] telegram local-control attachment send failed", {
            conversation: safeEnvelope.conversationKey,
            traceId: eventTraceId,
            reason: String((error as Error)?.message || error),
          });
      ...
      Logger.info("[Channels] local-control message processed", {
        runId: safeRunId,
        conversation: safeEnvelope.conversationKey,
        channel: jobChannel,
        ok: localControlResult.ok,
        actionCode: localControlResult.code,
    ...
    assistantMessageId = assistantPlaceholder.id;
    await withTimeoutGuard(
      "channel_update_run_assistant_message",
      ENFORCED_STORAGE_OPERATION_TIMEOUT_MS,
      async () => await storage.updateChatRunAssistantMessage(safeRunId, assistantMessageId!),
    ).catch((error) => {
      Logger.warn("[Channels] failed to attach assistant placeholder to run", {
        runId: safeRunId,
        conversation: safeEnvelope.conversationKey,
        traceId: eventTraceId,
        reason: String((error as Error)?.message || error),
    ...
    if (assistantMessageId) {
      await withTimeoutGuard(
        "channel_mark_assistant_message_failed",
        ENFORCED_STORAGE_OPERATION_TIMEOUT_MS,
        async () => await storage.updateChatMessageContent(
          assistantMessageId,
          aborted ? "[Flujo cancelado por mensaje nuevo]" : fallback,
          "failed",
          {
            runId,
            requestId: safeScopedRequestId,
    ...
    await withTimeoutGuard(
      "channel_mark_run_failed",
      ENFORCED_STORAGE_OPERATION_TIMEOUT_MS,
      async () => await storage.updateChatRunStatus(safeRunId, "failed", reason),
    ).catch(() => null);
    try {
      await patchConversationMetadata(conversation.id, {
        lastError: reason,
...
export async function processChannelIngestJob(job: ChannelIngestJob): Promise<void> {
  ...
  for (const [envelopeIndex, rawEnvelope] of envelopes.entries()) {
    ...
    // Telegram mirror should preserve conversation continuity and avoid
    // cancelling responses when the user sends messages in quick succession.
    // Keep queue order and only use "latest wins" abort on other channels.
...
export const __channelIngestServiceInternals = {
  sanitizeOutboundRichText,
