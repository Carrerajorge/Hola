...
import { B as resolveUserPath, c as CHANNEL_IDS, ft as resolvePreferredOpenClawTmpDir, i as defaultRuntime, q as truncateUtf16Safe, t as createSubsystemLogger } from "./subsystem-DDlKm7JR.js";
import { n as loadConfig, o as writeConfigFile, t as createConfigIO } from "./config-0er5lK-Y.js";
import { i as parseBooleanValue, t as formatCliCommand } from "./command-format-CSkgHwwD.js";
import { E as runExec, _ as DEFAULT_SOUL_FILENAME, b as ensureAgentWorkspace, f as DEFAULT_AGENTS_FILENAME, g as DEFAULT_IDENTITY_FILENAME, h as DEFAULT_HEARTBEAT_FILENAME, m as DEFAULT_BOOTSTRAP_FILENAME, n as resolveAgentConfig, p as DEFAULT_AGENT_WORKSPACE_DIR, u as resolveSessionAgentId, v as DEFAULT_TOOLS_FILENAME, y as DEFAULT_USER_FILENAME } from "./agent-scope-S0PcYLol.js";
import { n as formatErrorMessage, t as extractErrorCode } from "./errors-CYhifFbz.js";
import { n as isPathInside, t as isNotFoundPathError } from "./path-guards-B41Bkx5V.js";
import { b as openFileWithinRoot, i as getImageMetadata, n as buildImageResizeSideGrid, s as resizeToJpeg, t as IMAGE_REDUCE_QUALITY_STEPS, y as SafeOpenError } from "./image-ops-DukJmgIX.js";
import { t as SsrFBlockedError } from "./ssrf-DPlHrEn9.js";
import { t as sanitizeContentBlocksImages } from "./tool-images-DTG_6Hc_.js";
import { a as syncSkillsToWorkspace, h as safeEqualSecret, l as resolveSandboxInputPath, m as sanitizeEnvVars, u as resolveSandboxPath } from "./skills-5iUBhaef.js";
...
import { A as acquireSessionWriteLock, B as canonicalizeMainSessionAlias, V as resolveAgentMainSessionKey } from "./sessions-CUeRK6_V.js";
import { i as saveMediaBuffer, t as ensureMediaDir } from "./store-CC2fFkbE.js";
...
async function loadPwAiModule(mode) {
	try {
		return await import("./pw-ai-WAP29gR1.js");
	} catch (err) {
...
function createProfileContext(opts, profile) {
	...
	const resetProfile = async () => {
		...
		if (await isHttpReachable(300) && !profileState.running) try {
			await (await import("./pw-ai-WAP29gR1.js")).closePlaywrightBrowserConnection();
		} catch {}
...
function isPathInsidePosix$2(root, target) {
	if (root === "/") return true;
	return target === root || target.startsWith(`${root}/`);
}
function getOutsideAllowedRootsReason(sourceNormalized, allowedRoots) {
	if (allowedRoots.length === 0) return null;
	for (const root of allowedRoots) if (isPathInsidePosix$2(root, sourceNormalized)) return null;
	return {
...
function getReservedTargetReason(bind) {
	...
	for (const reserved of RESERVED_CONTAINER_TARGET_PATHS) if (isPathInsidePosix$2(reserved, target)) return {
...
function findMountByContainerPath(mounts, target) {
	for (const mount of mounts) if (isPathInsidePosix$1(mount.containerRoot, target)) return mount;
	return null;
}
...
function isPathInsidePosix$1(root, target) {
...
var SandboxFsBridgeImpl = class {
	...
	resolveMountByContainerPath(containerPath) {
		...
		for (const mount of this.mountsByContainer) if (isPathInsidePosix(normalizeContainerPath(mount.containerRoot), normalized)) return mount;
		return null;
	}
