...
const CHANNEL_SUBSYSTEM_PREFIXES = new Set(CHAT_CHANNEL_IDS);
...
export function createSubsystemLogger(subsystem) {
  ...
  function isConsoleEnabled(level) {
    const consoleSettings = getConsoleSettings();
    return (
  ...
  function isFileEnabled(level) {
    return isFileLogLevelEnabled(level);
  }

  function isEnabled(level, target = "any") {
    if (target === "console") {
