...
export function createAgenticTools(): ToolDefinition[] {
  ...
  const subagentStatus: ToolDefinition = {
    ...
    execute: async (input: any, context: ToolContext): Promise<ToolResult> => {
      ...
      if (!run || run.requesterUserId !== context.userId) {
        return fail("NOT_FOUND", "Subagent run not found");
      }
      return ok(run);
    },
  };
  ...
  const subagentCancel: ToolDefinition = {
    ...
    execute: async (input: any, context: ToolContext): Promise<ToolResult> => {
      ...
      return cancelled
        ? ok({ runId: input.runId, cancelled: true })
        : fail("CANNOT_CANCEL", `Run ${input.runId} cannot be cancelled`);
    },
  };
  ...
  return [spawnSubagent, subagentStatus, subagentList, subagentCancel, ragSearch, ragContext];
}
