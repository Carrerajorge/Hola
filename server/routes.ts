...
export async function registerRoutes(
  ...
  if (env.GOOGLE_CLIENT_ID && env.GOOGLE_CLIENT_SECRET) {
    ...
    app.get("/api/auth/google/callback",
      ...
      (req, res, next) => {
        if (req.session) {
          // Robustness: persist userId explicitly for OAuth logins too.
          // Some environments intermittently fail to hydrate req.user on subsequent requests.
          const u = req.user as any;
          const authUserId = u?.claims?.sub || u?.id;
          if (authUserId) {
  ...
  app.get("/api/connectors/:name/stats", (req: Request, res: Response) => {
    try {
      ...
      if (!isValidConnector(name)) {
        return res.status(400).json({
          success: false,
          error: `Invalid connector name: ${name}. Valid connectors: gmail, gemini, xai, openai, anthropic, deepseek, database, forms`,
        });
      }
  ...
  fileProcessingQueue.setProcessCallback(async (job) => {
    try {
      ...
      const embeddings = await ragPipeline.generateEmbeddingsBatch(texts);
