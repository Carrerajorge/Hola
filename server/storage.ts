...
export class MemStorage implements IStorage {
  ...
  async getChatMessages(chatId: string, options?: { limit?: number; offset?: number; before?: Date; orderBy?: 'asc' | 'desc' }): Promise<ChatMessage[]> {
    ...
    return cache.remember(cacheKey, 10, async () => {
      ...
      if (before) {
        conditions.push(sql`${chatMessages.createdAt} < ${before.toISOString()}`);
      }
      ...
      if (orderBy === 'desc') {
        ...
        query = query.orderBy(chatMessages.createdAt);
      }
  ...
  async createAuditLog(log: InsertAuditLog): Promise<AuditLog> {
    const [result] = await db.insert(auditLogs).values({ ...log, userId: undefined } as any).returning();
    return result;
  }
  ...
  async createToolCallLog(log: InsertToolCallLog): Promise<ToolCallLog> {
    const [result] = await db.insert(toolCallLogs).values({ ...log, userId: undefined } as any).returning();
    return result;
  }
  ...
  async createApiLog(log: InsertApiLog): Promise<ApiLog> {
    const [result] = await db.insert(apiLogs).values({ ...log, userId: undefined } as any).returning();
    return result;
  }
  ...
  async getAgentGapLogs(status?: string, userId?: string): Promise<AgentGapLog[]> {
    if (status && userId) {
      return dbRead.select().from(agentGapLogs)
        .where(and(eq(agentGapLogs.status, status), sql`1=1`))
