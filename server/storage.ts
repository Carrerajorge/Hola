...
export interface IStorage {
  ...
  updateFileChunkEmbeddingById(id: string, embedding: number[]): Promise<void>;
...
export class MemStorage implements IStorage {
  ...
  async updateFileChunkEmbeddingById(id: string, embedding: number[]): Promise<void> {
    await db.update(fileChunks)
      .set({ embedding })
      .where(eq(fileChunks.id, id));
  }
  ...
  async getChats(userId?: string): Promise<Chat[]> {
    if (userId) {
      // Default behavior: hide soft-deleted chats from the main list.
