...
export function createChatsRouter() {
  ...
  router.post("/chats", async (req, res) => {
    try {
      ...
      const chatHistoryEnabled = settings?.featureFlags?.chatHistoryEnabled ?? true;
      const chatInsert = {
        title: title || "New Chat",
        userId,
        ...(chatHistoryEnabled ? {} : { hidden: "true" as const }),
      };
      ...
      const chat = await storage.createChat(chatInsert);
      res.json(chat);
    } catch (error: any) {
  ...
  router.post("/chats/:id/messages", async (req, res) => {
    try {
      ...
      // Respect "Guardar historial de chat": when disabled, hide the chat from the main history list.
      // This provides "temporary chat" behavior without breaking the live conversation.
      if (userId) {
        const settings = await storage.getUserSettings(userId).catch(() => null);
        const chatHistoryEnabled = settings?.featureFlags?.chatHistoryEnabled ?? true;
        if (!chatHistoryEnabled && chat.hidden !== "true") {
          await storage.updateChat(chat.id, { hidden: "true" });
        }
      }
