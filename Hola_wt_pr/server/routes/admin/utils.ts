...
export async function requireAdmin(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const userEmailRaw = userReq.user?.claims?.email || 
                            userReq.user?.email ||
                            passportUser?.claims?.email || 
                            passportUser?.email ||
                            (req as any).user?.profile?.emails?.[0]?.value;
        const userEmail = typeof userEmailRaw === "string" ? userEmailRaw : undefined;
        const normalizedUserEmail = userEmail ? userEmail.toLowerCase().trim() : undefined;
        ...
        if (ADMIN_EMAIL && normalizedUserEmail && normalizedUserEmail === ADMIN_EMAIL) {
            isAdmin = true;
        ...
        if (!isAdmin) {
            ...
            const dbRole = String(dbUser?.role || "").toLowerCase().trim();
            isAdmin = SYSTEM_ADMIN_ROLES.has(dbRole);
            if (!isAdmin && ADMIN_EMAIL) {
                const dbEmail = String(dbUser?.email || "").toLowerCase().trim();
                if (dbEmail && dbEmail === ADMIN_EMAIL) {
                    isAdmin = true;
                    if (debugAuth) console.log("[Admin] Matched ADMIN_EMAIL via DB email");
                }
            }
            if (debugAuth) {
