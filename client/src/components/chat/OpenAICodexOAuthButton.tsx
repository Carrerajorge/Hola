...
export function OpenAICodexOAuthButton({ onConnected }: OpenAICodexOAuthButtonProps) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [open, setOpen] = React.useState(false);
  const [flowId, setFlowId] = React.useState<string | null>(null);
  const [authUrl, setAuthUrl] = React.useState("");
  const [redirectUri, setRedirectUri] = React.useState("http://localhost:1455/auth/callback");
  const [manualInput, setManualInput] = React.useState("");
  const [showManualFallback, setShowManualFallback] = React.useState(false);
  const popupRef = React.useRef<Window | null>(null);
  const lastHandledFlowStateRef = React.useRef<"completed" | "failed" | null>(null);

  const { data: status, isLoading: isStatusLoading } = useQuery<OpenAICodexStatusResponse>({
    queryKey: STATUS_QUERY_KEY,
    queryFn: async () => {
      const res = await apiFetch("/api/oauth/openai/codex/status", { cache: "no-store" });
      if (!res.ok) {
  ...
  const flowQuery = useQuery<OpenAICodexFlowResponse>({
    ...
    queryFn: async () => {
      const res = await apiFetch(`/api/oauth/openai/codex/flow/${flowId}`, { cache: "no-store" });
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(payload?.error || "No se pudo consultar el flujo ChatGPT OAuth");
      }
      return payload;
    },
    ...
    onSuccess: (payload) => {
      ...
      if (!popup) {
        ...
        toast({
          ...
          description: "Abre manualmente la URL de ChatGPT o habilita popups para continuar.",
        });
        return;
      }
      popup.focus();
  ...
  const completeMutation = useMutation<void, Error, { flowId: string; input: string }>({
  ...
  React.useEffect(() => {
    ...
    if (flowState.status === "failed") {
      ...
      toast({
        ...
        description: flowState.error || "No se pudo finalizar la autenticación.",
        variant: "destructive",
      });
      return;
    }
    ...
    void (async () => {
      ...
      await Promise.resolve(onConnected?.(flowState.result?.defaultModelId || "gpt-5.3-codex"));
      toast({
  ...
  const handleOpenAuthUrl = React.useCallback(() => {
    ...
    if (!popup) {
      ...
      toast({
        title: "Ventana bloqueada",
        description: "No se pudo abrir ChatGPT automáticamente. Usa la recuperación manual.",
      });
      return;
    }
    popupRef.current = popup;
  ...
  return (
    <>
      ...
      >
        <ChatGptLogoIcon className={isConnected ? "text-emerald-600" : "text-foreground"} />
      </Button>
      ...
      <Dialog open={open} onOpenChange={resetLocalState}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Usa tu cuenta de <strong>ChatGPT Plus/Pro</strong> para habilitar GPT Codex en ILIAGPT.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="rounded-2xl border border-border/70 bg-muted/30 p-4 text-sm">
              {isStatusLoading ? (
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-muted-foreground">
                    Al terminar este flujo se habilitará <code className="rounded bg-background px-1 py-0.5 text-xs">openai-codex/gpt-5.3-codex</code>.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="space-y-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4">
                <div className="space-y-2 text-sm">
                  ...
                  <ol className="space-y-1 text-muted-foreground">
                    ...
                    <li>2. Si este servidor corre localmente, la vinculación se cerrará sola.</li>
                    <li>3. Si ves una URL que termina en <code>{redirectUri}</code>, copia esa URL completa y pégala abajo.</li>
                  </ol>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    {showManualFallback ? "Ocultar recuperación manual" : "Mostrar recuperación manual"}
                  </Button>
                </div>

                {showManualFallback ? (
                  <div className="space-y-2">
                    <label htmlFor="openai-codex-manual" className="text-sm font-medium">
                  ...
                  <div className="rounded-xl border border-border/60 bg-background/70 p-3 text-sm text-muted-foreground">
                    Esperando la confirmación de ChatGPT. Si no se completa, abre la recuperación manual.
                  </div>
                )}
              </div>
            ) : null}
          ...
          <DialogFooter>
            ...
            {flowId ? (
                  ...
                  onClick={() => {
                    ...
                    if (isOpenAIAuthorizationUrl(normalizedInput)) {
                      toast({
                        ...
                        description: "Pega la URL final de localhost o solo el code, no la URL inicial de OpenAI.",
                        variant: "destructive",
                      });
                      return;
                    }
              ...
              <Button type="button" onClick={() => startMutation.mutate()} disabled={isBusy}>
