...
import{Wa as r,d as i}from"./chunk-IN4OQ7I6.mjs";import{eq as o,sql as f}from"drizzle-orm";var l={free:{dailyRequests:3,model:"grok-4-1-fast-non-reasoning"},go:{dailyRequests:50,model:"grok-4-1-fast-non-reasoning"},plus:{dailyRequests:200,model:"grok-4-1-fast-non-reasoning"},pro:{dailyRequests:-1,model:"grok-4-1-fast-non-reasoning"},admin:{dailyRequests:-1,model:"grok-4-1-fast-non-reasoning"}},d=process.env.ADMIN_EMAIL||"";function q(){let y=new Date,s=new Date(y);return s.setDate(s.getDate()+1),s.setHours(0,0,0,0),s}var g=class{async checkAndIncrementUsage(s){let[e]=await r.select().from(i).where(o(i.id,s));if(!e)return{allowed:!1,remaining:0,limit:0,resetAt:null,plan:"free",message:"Usuario no encontrado"};let t=d&&e.email?.toLowerCase()===d.toLowerCase()||e.role==="admin",a=t?"admin":e.plan||"free",n=l[a]||l.free;if(t||n.dailyRequests===-1)return{allowed:!0,remaining:-1,limit:-1,resetAt:null,plan:a};let c=new Date,_=q(),u=await r.execute(f`
      UPDATE users
      SET
