...
- **Router System**: Hybrid decision system that automatically routes messages between chat (simple responses) and agent mode (multi-step tasks with tools). Uses a cascade: (1) Heuristic patterns for quick detection, (2) ComplexityAnalyzer fallback, (3) LLM router for ambiguous cases. Configuration via environment variables: `ROUTER_CONFIDENCE_THRESHOLD` (default 0.65), `MAX_AGENT_STEPS` (default 8), `ENABLE_DYNAMIC_ESCALATION` (default true). Endpoints: POST /api/chat/route, POST /api/chat/agent-run, POST /api/chat/escalation-check.
- **AgentRunner**: Simplified agent loop for executing multi-step tasks. Supports 4 tools: `web_search`, `open_url`, `extract_text`, `final_answer`. Features heuristic fallback when LLM is unavailable, event emission for progress tracking, and configurable max steps.
