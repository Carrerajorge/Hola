  ...
  router.post("/chat/stream", async (req, res) => {
    ...
    try {
      ...
      if (lastImageBase64 && typeof lastImageBase64 === "string") {
        ...
        console.log(`[Stream] 🖼️ Vision: injecting lastImageBase64 (${Math.round(lastImageBase64.length / 1024)}KB)`);
      }
      ...
      if (imagePartsForVision.length > 0) {
        ...
        for (let i = formattedMessages.length - 1; i >= 0; i--) {
          if (formattedMessages[i].role === "user") {
            ...
            console.log(`[Stream] 🖼️ Vision: converted last user message to multimodal (${imagePartsForVision.length} images)`);
            break;
          }
        }
        ...
        if (latencyMode === 'fast') {
          ...
          console.log(`[Stream] 🖼️ Vision: upgraded latency mode to 'deep' for image analysis`);
        }
      }
