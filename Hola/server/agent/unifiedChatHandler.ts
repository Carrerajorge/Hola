...
export async function createUnifiedRun(
  ...
  // our reservation clarification, the current message alone won't match
  // web_automation patterns. Check if the previous assistant message was
  // a reservation clarification and if the current message contains
  // contact-like info (name, phone, email).
  if (requestSpec.intent !== "web_automation") {
    ...
    if (isReservationFollowUp) {
      // The user is likely providing missing reservation details
      ...
      if (hasContactInfo) {
        console.log(`[UnifiedChat] Reservation follow-up detected — overriding intent from "${requestSpec.intent}" to "web_automation"`);
