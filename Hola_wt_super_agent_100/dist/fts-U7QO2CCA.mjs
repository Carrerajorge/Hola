...
import{Wa as c,hb as o}from"./chunk-SFS5HO6Q.mjs";import{h as s,i as r}from"./chunk-H7EJRODP.mjs";import"./chunk-2FP5DEJW.mjs";o();r();async function _(){let e=await c.connect();try{s.info("[FTS] Starting Full-Text Search setup..."),await e.query(`
    ...
    `);let{rows:t}=await e.query("SELECT COUNT(*) as count FROM chat_messages WHERE search_vector IS NULL"),a=parseInt(t[0].count,10);a>0?(s.info(`[FTS] Backfilling ${a} messages with null search_vector...`),await e.query(`
      ...
      `),s.info("[FTS] Backfill complete.")):s.info("[FTS] No backfill needed."),s.info("[FTS] Setup complete.")}catch(t){s.error("[FTS] Setup failed:",t.message)}finally{e.release()}}export{_ as setupFts};
