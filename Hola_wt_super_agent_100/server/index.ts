...
(async () => {
  ...
  app.use((req, _res, next) => {
    ...
    // Attach lightweight device/session metadata for "trusted devices" UX.
    ...
    if (session) {
      ...
      if (!session.device.createdAt) session.device.createdAt = now;
      session.device.userAgent = req.headers["user-agent"] || null;
      session.device.ip = req.ip || req.socket.remoteAddress || null;
      if (!session.device.lastSeenAt || now - session.device.lastSeenAt > 60_000) {
        session.device.lastSeenAt = now;
      }

      // Bind session to authenticated user for simpler secure queries later.
      if (userId && !userId.startsWith("anon_") && !session.authUserId) {
