...
export function createUserRouter() {
  ...
  router.put("/api/users/:id/privacy", async (req, res) => {
    try {
      ...
      const trainingOptIn = parseBoolean(req.body?.trainingOptIn);
      const remoteBrowserDataAccess = parseBoolean(req.body?.remoteBrowserDataAccess);

      if (trainingOptIn === undefined && remoteBrowserDataAccess === undefined) {
        return res.status(400).json({ error: "No valid privacy fields provided" });
      }

      const existing = await storage.getUserSettings(id);
      const currentPrivacy = existing?.privacySettings || { trainingOptIn: false, remoteBrowserDataAccess: false };
      const nextPrivacy = {
        ...currentPrivacy,
        ...(trainingOptIn !== undefined ? { trainingOptIn } : {}),
        ...(remoteBrowserDataAccess !== undefined ? { remoteBrowserDataAccess } : {}),
      };
